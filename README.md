...
웹앱을 `http://localhost:5173` 또는 `http://127.0.0.1:5173`으로 열고 **내장 카메라 연결**을 누르면
브라우저가 카메라 권한을 요청합니다. 권한을 허용하면 눈 추적용 카메라의 실시간 미리보기를 확인할 수 있습니다.

현재 카메라 연결은 UI와 카메라 구도를 점검하기 위한 브라우저 미리보기 모드입니다. 이후 MediaPipe 시선 분석을
Python에 연결할 때는 Python 영상 스트림 모드로 전환하며, 브라우저 미리보기와 Python이 같은 카메라를 동시에
열지 않도록 구성합니다.
